...
- <h4>Product</h4> : 전국 캠핑장, 각 지역 캠핑 존

- <h4>Targeting End User</h4> : 꼭 휴가철만이 아니더라도, 가족들과 함께 떠나고 싶은 직장인들, 
...
- 프로젝트에서 스프린트 미팅을 2번 실행하여 각 스프린트 기간동안 목표%를 설정하고 진행, 스프린트 기간이 지나면 부족한 부분 보완하는 회의 진행
...
- 결제 전 사용자가 구매하고자 하는 일정/위치를 한번에 확인할 수 있도록 정보 제공
- 간편결제 시 필수로 있어야 하는 redirect 페이지에 "결제 중" 이라는 애니메이션을 적용하여 결제완료 API가 도착할 시간동안 페이지가 이동하는 과정에서 고객이 혼란이 생기지 않도록 함
...
- 추가적인 데이터 활용을 위해 백엔드에 각 별점을 1~5로 수치화 하여 전달
...
- 로그아웃 버튼 클릭 시 localStorage에 저장되어있는 고객 token값을 삭제하여 로그아웃 진행
